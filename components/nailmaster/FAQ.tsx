...
import { useFAQ, useContent } from '@/context/SiteDataContext'
...
export default function FAQ() {
  ...
  const title = content?.faq_title || 'Часто задаваемые вопросы'

  const toggleItem = (index: number) => {
    setActiveIndex(activeIndex === index ? null : index)
  }

  // Default FAQ items if none loaded
  const faqItems = faq.length > 0 ? faq : [
    {
        ...
        <div className="max-w-[800px] mx-auto">
          {isLoading ? (
            ...
            Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="bg-background rounded-xl mb-4 p-6 animate-pulse">
